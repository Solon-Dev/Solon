...
async function callClaudeAPI(diff: string, playbooks: Playbook[], langConfig: LanguageConfig): Promise<ReviewResult | ErrorResult> {
  try {
    ...
    // Log stack trace server-side but do not expose to client
    if (errorStack) {
      console.error('Claude API Error Stack:', errorStack);
    }
...
`;
    ...
    if (errorStack) {
      console.error('Internal Server Error Stack:', errorStack);
    }
