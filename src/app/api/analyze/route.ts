...
async function callClaudeAPI(diff: string, playbooks: Playbook[], langConfig: LanguageConfig): Promise<ReviewResult | ErrorResult> {
  try {
    ...
    // SENTINEL SECURITY FIX: Stack traces should not be exposed to the client
    // const errorStack = error instanceof Error ? error.stack : undefined;
    console.error('Claude API Error:', error); // Log it server-side instead

    return { 
      error: `Claude API analysis failed: ${errorMessage}`
      // stack: errorStack // Removed to prevent information leakage
    };
  }
}
...
`;
    ...
    // SENTINEL SECURITY FIX: Do not leak stack traces to client
    // const errorStack = error instanceof Error ? error.stack : undefined;
    console.error('API Handler Error:', error); // Log server-side

    return NextResponse.json(
      { 
        error: "Internal server error",
        details: errorMessage,
        // stack: errorStack, // Removed to prevent info leak
        diagnostics: {
          timestamp: new Date().toISOString(),
          hasApiKey: !!process.env.ANTHROPIC_API_KEY
        }
