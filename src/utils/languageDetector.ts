...
// Hoisted regex to avoid recompilation on every function call
const FILE_PATH_REGEX = /^(?:diff --git|---|\+\+\+) [ab]\/(.+)$/gm;

// Hoisted configuration objects to prevent re-creation
...
const BEST_PRACTICES: Record<SupportedLanguage, string> = {
  javascript: `
...
const SECURITY_CONSIDERATIONS: Record<SupportedLanguage, string> = {
  javascript: `
- XSS vulnerabilities
- SQL/NoSQL injection
- Prototype pollution
...
export function getLanguageBestPractices(language: SupportedLanguage): string {
  return BEST_PRACTICES[language];
}
...
export function getLanguageSecurityConsiderations(language: SupportedLanguage): string {
  return SECURITY_CONSIDERATIONS[language];
}
