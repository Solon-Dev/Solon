...
export function detectLanguageFromDiff(diff: string): SupportedLanguage {
  ...
  // Track which languages have been found
  const foundLanguages = new Set<SupportedLanguage>();

  // Optimization: Single pass processing with early exit for mixed content
  ...
  // If we reach here, foundLanguages.size is exactly 1
