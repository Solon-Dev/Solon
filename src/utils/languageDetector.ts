...
export function detectLanguageFromDiff(diff: string): SupportedLanguage {
  ...
  // Single pass loop with early exit for mixed content
  while ((match = filePathRegex.exec(diff)) !== null) {
    if (match[1]) {
      ...
      if (detected) {
        detectedLanguages.add(detected);

        // Early exit if more than one language is detected
  ...
  // Return the single detected language (since size > 1 returns mixed early)
  // We can just return the first (and only) item in the set
  return detectedLanguages.values().next().value;
}
